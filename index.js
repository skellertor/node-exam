...
const cachedFile = 'cached.json',
      dataDirectory = 'data',
      cachedDirecory = 'cached',
      tagsFile = 'tags.txt';
...
lib.getCached(path.join(__dirname, cachedDirecory, cachedFile ), function (err, cached) {
    if(err){
        // 2. Gets all filenames in data directory
        lib.getFileNames(path.join(__dirname, dataDirectory), function (filenames) {
            ...
            lib.getTags(filenames, function (tags) {
                ...
                lib.countTags(tags, function (countedTags) {
                    ...
                    lib.cacheResults(countedTags, path.join(__dirname, cachedDirecory), function () {
                        // 6. Grabs the args from the CLI or uses default `tags.txt` if none provided
                        lib.getArgs(args, path.join(__dirname, tagsFile), function (finalArgs, longestArg) {
